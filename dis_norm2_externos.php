...
include "coneccion_i.php";
	...
	$q_BC = "SELECT id_evaluador FROM externos_360 
	where terminado=1 and revision=$revision order by id";
...
$q_BC2 = "SELECT STDDEV_POP(tex_calificaciones.calificacion) as de, avg(tex_calificaciones.calificacion) as pro FROM `tex_calificaciones`   WHERE tex_calificaciones.id_evaluador=$res2[0] and revision=$revision group by tex_calificaciones.id_evaluador";
		...
		{		
			...
			$q_BC3= "update externos_360 set desviacion=$ds, promedio=$avr where id_evaluador=$res2[0] and revision=$revision";  
			
			$r_BC3= mysqli_query($enlace,$q_BC3) or die("La consulta fall&oacute;P1: $q_BC2". mysqli_error($enlace) );
			$q_BC32= "SELECT calificacion, id_evaluado, id_competencia from tex_calificaciones where id_evaluador=$res2[0] and revision=$revision";
			...
			$pregunta=$res3[2];
				...
				$q_BC4 = "update tex_calificaciones set normalizado=$res where id_evaluado=$res3[1] and id_evaluador=$res2[0] and id_competencia=$pregunta and revision=$revision";  
