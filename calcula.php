...
$q_BC2 = "SELECT id_evaluado, avg(resultadon)*150 FROM new_resultados where id_evaluado=$res2[0] and revision=$revision";
		...
		while(@mysqli_num_rows($r_BC2)>=$count1)
		{
			...
			$cons = "UPDATE planes set  resultado_total=(resultado_360*.4)+(resultado_obj_final*.6) where id_empleado=$res2[0] and revision=$revision";
			$rest= mysqli_query($enlace,$cons) or die("La consulta fall&oacute;P1: $cons". mysqli_error($enlace));
