...
include "coneccion_i.php";
	...
	where usuarios.activo=1 and planes.estatus_360=3 and planes.revision=$revision order by id";
...
$q_BC2 = "SELECT STDDEV_POP(tex_calificaciones.calificacion) as de, avg(tex_calificaciones.calificacion) as pro FROM `tex_calificaciones`   WHERE tex_calificaciones.id_evaluador=$res2[0] group by tex_calificaciones.id_evaluador";
		...
		{		
			...
			$q_BC3= "update planes set desviacion=$ds, promedio=$avr where id_empleado=$res2[0]";  
			$r_BC3= mysqli_query($enlace,$q_BC3) or die("La consulta fall&oacute;P1: $q_BC2". mysqli_error($enlace) );
			$q_BC32= "SELECT calificacion, id_evaluado, id_competencia from tex_calificaciones where id_evaluador=$res2[0]";
